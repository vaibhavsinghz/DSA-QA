...
class Solution
{
    ...
    void travel(Node* root, vector<pair<Node*, int>> &nodelvl, int lvl){
    ...
    {
       vector<pair<Node*, int>> nodelvl;
        travel(root, nodelvl, 1);
        int ppos, qpos;
        for(int i = 0; i < nodelvl.size(); ++i){
            if(nodelvl[i].first -> data == p) ppos = i;
            if(nodelvl[i].first -> data == q) qpos = i;
        }
